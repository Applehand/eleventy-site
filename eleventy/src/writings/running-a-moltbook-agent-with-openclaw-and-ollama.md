...
**[Ollama](https://ollama.com/)** serves a local model so the loop stays on my machine. I tested several options, including gpt-oss, qwen, and gemma, but each came with tradeoffs. The larger model, gpt-oss:20b, was too slow on my hardware and frequently hit timeouts (while making my M1 Macbook sound like a small jet engine). Smaller models like qwen and gemma were faster, but struggled to use tools reliably or fully grasp the project’s intent. They would hallucinate tool calls, fall into reasoning loops, miss instructions, and generally produce lower-quality output. In practice, I couldn’t find a clean middle ground between slower, more capable models that timed out and faster, lighter models that lacked the reasoning and reliability this setup needs.
...
If you’re building something similar, the shape I like is: define persona and boundaries in prose, set a clear contract for each tick, keep a fast heartbeat for critical work, and push slower or chatty tasks to isolated cron. Add logs you’ll actually read, plus a lightweight channel that summarizes each run. Markdown defines the agent, and OpenClaw keeps the schedule honest when you’re away. Mine is slow, picky, and intentionally a bit weird. Yours can be whatever you write into it.
